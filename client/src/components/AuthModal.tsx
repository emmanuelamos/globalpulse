...
export function AuthModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  // Prevent hydration mismatch by only rendering portal on the client
  ...
  // If closed or not mounted on client yet, render nothing
  if (!isOpen || !mounted) return null;

  // 🛑 NEW: Wrap the entire JSX in createPortal
  return createPortal(
    // Notice the z-[9999] to ensure it covers your Live Feed ticker!
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      {/* We added max-h-[90vh] and overflow-y-auto so if the screen 
        is small, the modal scrolls instead of getting cut off 
      */}
      <div className="w-full max-w-md max-h-[90vh] overflow-y-auto rounded-2xl bg-white p-8 shadow-2xl relative">
    ...
    document.body // <--- Teleports the UI to the root of the page
  );
}